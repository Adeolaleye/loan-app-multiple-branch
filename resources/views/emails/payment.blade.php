...
@if( $data['type'] == 'partial payment')

<p>Hello Super Admin,</p>
<p>
    A partial payment has just been made by client with <strong>ID No: </strong>{{ $data['client_no'] }}, below are details of the payment:<br> 
    <strong>Client ID : </strong>{{ $data['client_no'] }}<br>
    <strong>Client Name : </strong> {{ $data['name'] }}<br>
    <strong>Client Phone no : </strong>{{ $data['phone'] }}<br>
</p>
...
<p>
    <strong>Loan Amount : </strong>&#x20A6;{{ number_format($data['loan_amount']) }}<br>
    <strong>Total Payback : </strong>&#x20A6;{{ number_format($data['total_payback']) }}<br>
    <strong>Total Amount Paid : </strong>&#x20A6;{{ number_format($data['total_amountpaid']) }} (this pay included)
</p>
<h4>Payment Details</h4>
<p>
    <strong>Amount Paid: </strong>&#x20A6;{{ number_format($data['amount_paid']) }}<br>
...
<p>
    <strong>Outstanding Pay : </strong>&#x20A6;{{ number_format($data['outstanding']) }}<br>
    <strong>To Balance : </strong>&#x20A6;{{ number_format($data['next_pay']) }}<br>
    <strong>Due Date : </strong>{{ date('d, M Y', strtotime($data['next_due_date'])) }}<br>
</p>
<p>Click <a href="" target="_blank">here</a> to login and review, login on desktop for best review.</p>
